...
{
	(CVAR = CreateConVar("sm_swgm_shop_bonus_type",		"1",	"0 - Credits are multiplied 1 - Credits are added", _, true, 0.0, true, 1.0)).AddChangeHook(OnTypeChange);
	g_bType = CVAR.BoolValue;
	
	(CVAR = CreateConVar("sm_swgm_shop_bonus_amount",	"2.0",	"Number of credits added / multiplied", _, true, 0.0)).AddChangeHook(OnAmountChange);
	...
	(CVAR = CreateConVar("sm_swgm_shop_bonus_interval",	"60.0",	"Break to earn credits", _, true, 0.0)).AddChangeHook(OnIntervalChange);
	g_fInterval = CVAR.FloatValue;
	
	(CVAR = CreateConVar("sm_swgm_shop_bonus_playing",	"1",	"0 - Grants credits to everyone 1 - Grants no credits to spectators", _, true, 0.0, true, 1.0)).AddChangeHook(OnPlayChange);
	g_bPlay = CVAR.BoolValue;
	
	(CVAR = CreateConVar("sm_swgm_shop_bonus_discount",		"0.5",	"Discount on items in the shop")).AddChangeHook(OnDiscChange);
